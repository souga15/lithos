...
const About = () => {
  const roadmap = [
    ...
    { year: 'PHASE 7',   title: 'LITHOS WebApp',  body: 'Deployment of full-stack PWA with community hazard reporting and live API pipes.', status: 'COMPLETED', icon: <Mountain className="w-5 h-5" /> },
    { year: 'PHASE 10',  title: 'PINN AI Engine', body: 'Deployment of Physics-Informed Neural Networks and high-resolution slope-unit mapping.', status: 'ACTIVE',    icon: <Cpu className="w-5 h-5" /> }
  ];

  const features = [
    { title: 'Physics-Informed AI',       desc: 'Trained using the Infinite Slope Model as a loss constraint, the PINN ensures all predictions strictly obey geotechnical physics.',     icon: <BrainCircuit className="w-5 h-5" /> },
  ...
  const techStack = ['Sentinel-1', 'Sentinel-2', 'NASA GPM', 'React.js', 'FastAPI', 'PyTorch (PINN)', 'XGBoost', 'CesiumJS', 'Leaflet', 'GeoPandas'];
    ...
    <div className="min-h-screen bg-bg text-white relative overflow-x-hidden pb-24">
      ...
      >
        <span className="data-mono text-[10px] text-white/15 uppercase tracking-[0.4em]">
          LITHOS v10.0.0 &copy; 2026 · Built for Resilience
        </span>
      </footer>
    </div>
  );
