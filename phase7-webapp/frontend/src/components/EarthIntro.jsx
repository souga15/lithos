...
function initThreeEarth(container, onReady) {
  const ThreeLib = THREE || window.THREE;
  if (!ThreeLib || !container) return null;
  ...
  const camera = new ThreeLib.PerspectiveCamera(45, W / H, 0.1, 1000);
  camera.position.set(0, 0, 2.6);

  // ── Lights ──
  const sun = new ThreeLib.DirectionalLight(0xffeedd, 2.2);
  sun.position.set(5, 3, 5);
  scene.add(sun);
  scene.add(new ThreeLib.AmbientLight(0x111133, 0.5));
  const rimLight = new ThreeLib.DirectionalLight(0x4488ff, 0.4);
  ...
  // ── Earth Mesh ──
  const earthRadius = 1;
  const earthGeo = new ThreeLib.SphereGeometry(earthRadius, 64, 64);
  const earthMat = new ThreeLib.MeshPhongMaterial({ 
    color: 0x1a3a6c,
    shininess: 18 
  });
  
  const earthGroup = new ThreeLib.Group();
  const earth = new ThreeLib.Mesh(earthGeo, earthMat);
  ...
  // ── Cloud Layer ──
  const cloudGeo = new ThreeLib.SphereGeometry(1.012, 64, 64);
  const cloudMat = new ThreeLib.MeshPhongMaterial({ transparent: true, opacity: 0.38, depthWrite: false });
  const clouds = new ThreeLib.Mesh(cloudGeo, cloudMat);
  earthGroup.add(clouds);

  // ── Atmosphere glow ──
  const atmGeo = new ThreeLib.SphereGeometry(1.08, 64, 64);
  const atmMat = new ThreeLib.MeshPhongMaterial({
    color: 0x4488ff,
    transparent: true,
    opacity: 0.10,
    side: ThreeLib.FrontSide,
    depthWrite: false,
  });
  const atm = new ThreeLib.Mesh(atmGeo, atmMat);
  scene.add(atm);

  // Notify ready so sphere appears immediately
  if (onReady) onReady();
  ...
  loader.load(SPECULAR_TEX, tex => {
    ...
    earthMat.specular = new ThreeLib.Color(0x333333);
    earthMat.needsUpdate = true;
  });
  ...
  loader.load(NIGHT_TEX, tex => {
    ...
    earthMat.emissive = new ThreeLib.Color(0xffaa44);
...
const EarthIntro = ({ regions, onSelect }) => {
  ...
  useEffect(() => {
    ...
    function startScene() {
      if (!earthRef.current || !isMounted) return;
      const scene = initThreeEarth(earthRef.current, () => {
        if (isMounted) setEarthReady(true);
      });
      if (!scene) return;
